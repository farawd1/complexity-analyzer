"""
Quick trainer - minimal version that trains for just 3 epochs on CPU.
"""
import torch
...
def main():
    print("=" * 60)
    print("C++ Complexity Analyzer — Training Pipeline (Fast)")
    print("=" * 60)
    ...
    tokenizer = CppTokenizer(max_vocab=8000, max_seq_len=256)
    tokenizer.build_vocab([c for c, _ in data])
    tokenizer.save("checkpoints/tokenizer.json")
    print(f"Vocab size: {tokenizer.vocab_size}")

    print("\n[3/5] Creating dataloaders (batch_size=128)...")
    parser = CppASTParser()
    train_loader, val_loader, test_loader = get_dataloaders(
        data, tokenizer, parser, batch_size=128)

    print("\n[4/5] Building model (minimal)...")
    model = ComplexityTransformer(
        vocab_size=tokenizer.vocab_size, d_model=64, n_heads=4,
        n_layers=2, d_ff=128, max_seq_len=256, dropout=0.1)
